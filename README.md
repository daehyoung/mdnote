...
본 프로젝트는 AI 에디터와 관리자(사람)가 긴밀하게 협업하는 **신뢰 기반의 문서 주도 개발(Document-Driven Development)** 프로세스를 따릅니다. 이는 소프트웨어 엔지니어링의 **나선형(Spiral) 개발 방법론(애자일)**을 기반으로 하며, 이를 AI와 사람의 상호작용 방식으로 확장하여 반복적인 개선과 위험 관리를 정착시킨 것입니다.

1.  **요구사항 및 설계 요청 (Planning)**: 관리자(사람)가 새로운 기능이나 변경 사항을 요청하면, AI는 관련 문서를 먼저 작성하거나 업데이트하여 구현 방향을 제안합니다.
2.  **문서 기반 구현 (Implementation)**: 합의된 설계 문서(`01~04` 계열)를 바탕으로 AI가 실제 소스 코드를 생성하고 리팩토링을 수행합니다.
